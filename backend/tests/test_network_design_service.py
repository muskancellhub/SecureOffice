...
class TestNetworkDesignService(unittest.TestCase):
    ...
    def test_update_diagram_persists_xml_only(self):
        # BUG-DES-006: saving the diagram updates drawio_xml without touching
        # the rest of the design.
        design = self.service.save_design(
            self.actor,
            {'submit': False, 'status': 'draft',
             'calculator_input': {'totalUsers': 42}},
        )
        updated = self.service.update_diagram(
            self.actor, str(design.id), '<mxGraphModel>edited</mxGraphModel>')
        self.assertEqual(updated.drawio_xml, '<mxGraphModel>edited</mxGraphModel>')
        # other fields are untouched
        self.assertEqual(updated.calculator_input_json, {'totalUsers': 42})

    def test_autogenerated_name_is_client_facing(self):
        # BUG-DES-005: no more design{N}-INITIALS-slug-DATE; a readable label.
        design = self.service.save_design(self.actor, {'submit': False, 'status': 'draft'})
        self.assertTrue(design.design_name.startswith('Network Design #'))
        self.assertNotRegex(design.design_name, r'^design\d+-')
